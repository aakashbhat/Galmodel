...
def bulgerot(R,Mb,bb,Ro):
    vr1=(R-Ro)/pow((pow((R-Ro),2)+bb*bb),1.5)
    vr2=(R+Ro)/pow((pow((R+Ro),2)+bb*bb),1.5)
    return (Mb*R*(vr1+vr2))
...
def bulgevert(R,z,Mb,bb,Ro):
    num=Mb*z
    den1=pow((R-Ro),2)+z*z+bb*bb
    den1=pow(den1,1.5)
    den2=pow((R+Ro),2)+z*z+bb*bb
    den2=pow(den2,1.5)
    vertf=(num/den1)+(num/den2)
    return vertf
...
data1 = ascii.read(path+"rotcurv_eilers.ascii")
data2 = ascii.read(path+"rotcurv_bhatta_25kpc.ascii")

if Inner==1:
    name="Reid(2014)"
    data3 = ascii.read(path+"rotcurv_reid_1to5.ascii")
elif Inner==2:
    name="Sofue(2021)"
    data3 = ascii.read(path+"rotcurv_sofue_2021_5kpc.ascii")
elif Inner==3:
    name="Sofue(2009)"
    data3 = ascii.read(path+"rotcurv_sofue_2009_5kpc.ascii")
...
def rotcurve(R,mb,bb,Ro,md,ad,bd,mh,ah):
        vr2tot=bulgerot(R,mb,bb,Ro)+diskrot(R,0,md,ad,bd)+halorot(R,mh,ah)
        return 10*pow(vr2tot,0.5)
def vertforce(R,z,mb,bb,Ro,md,ad,bd,mh,ah):
    vertftot=bulgevert(R,z,mb,bb,Ro)+diskvert(R,z,md,ad,bd)+halovert(R,z,mh,ah)
...
def residual(params, x1,x2, vel, uncertainty1,vertif,erforce):
        ...
        ah= params['ah']
        Ro=params['Ro']
        model1 = rotcurve(x1,mb,bb,Ro,md,ad,bd,mh,ah)
        model2 = vertforce(x2,1.1,mb,bb,Ro,md,ad,bd,mh,ah)
...
params.add('Ro', value=0.1,min=0,max=5)
...
#out2=mini.minimize(method="ampgo",params=out.params)
#print(fit_report(out2))
out3=mini.minimize(method="emcee",params=out.params,steps=steps,is_weighted=True)

print(fit_report(out3))
file=open('fitreports/fitreport_model4_vertf_rotcurve_%s_%s.dat'%(name,steps),'w')
...
for name2 in params.keys():
    ...
    file.write("\n")
    fmt = '{:8.4f} {:8.4f} {:8.4f} {:8.4f} {:8.4f}'.format
    file.write(fmt(mode["mode"],mode["HDI_lo"],mode["HDI_hi"],(mode["mode"]-mode["HDI_lo"]),(mode["HDI_hi"]-mode["mode"])))
    file.write("\n")
...
vrmodel_mcmc=rotcurve(radiusformodel,mcmc_mod.params['mb'].value,mcmc_mod.params['bb'].value,mcmc_mod.params['Ro'].value,mcmc_mod.params['md'].value,mcmc_mod.params['ad'].value,mcmc_mod.params['bd'].value,mcmc_mod.params['mh'].value,mcmc_mod.params['ah'].value)#Rotation curve for our best fit
vrmodel_ampgo=rotcurve(radiusformodel,out.params['mb'].value,out.params['bb'].value,out.params['Ro'].value,out.params['md'].value,out.params['ad'].value,out.params['bd'].value,out.params['mh'].value,out.params['ah'].value)#Rotation curve for our best fit


vertmodel_mcmc=vertforce(radiusforvertf,1.1,mcmc_mod.params['mb'].value,mcmc_mod.params['bb'].value,mcmc_mod.params['Ro'].value,mcmc_mod.params['md'].value,mcmc_mod.params['ad'].value,mcmc_mod.params['bd'].value,mcmc_mod.params['mh'].value,mcmc_mod.params['ah'].value)#Vertical force for our best fit 
vertmodel_ampgo=vertforce(radiusforvertf,1.1,out.params['mb'].value,out.params['bb'].value,out.params['Ro'].value,out.params['md'].value,out.params['ad'].value,out.params['bd'].value,out.params['mh'].value,out.params['ah'].value)#Vertical force for our best fit 
...
plt.errorbar(np.log10(radius3), vel3, yerr=(poser3,neger3),fmt='x', label=name)
plt.legend()
plt.savefig("Plots/%s/Rotcur_ModelIV_eil_bhatta_log_%s.pdf"%(name,steps))
###RotCurve:
plt.figure(1)
plt.xlabel('R (kpc)')
plt.ylabel('V$_{circ}$')
...
plt.legend()
plt.savefig("Plots/%s/Rotcur_ModelIV_eil_bhatta_%s.pdf"%(name,steps))


####Residuals (Use log for radius when working with rotation curve):
plt.figure(2)
plt.errorbar(radius4, out.residual[len(radius):],fmt='+', label='Ampgo Residual')
plt.axhline(y=0,color='r')
plt.xlabel('R(kpc)')
plt.ylabel('Residuals')
plt.savefig("Plots/%s/Residuals_vertif_%s.pdf"%(name,steps))
###Vertical Force:
plt.figure(3)
plt.xlabel('R (kpc)')
plt.ylabel('F$_{v}$')
...
plt.legend()
plt.savefig("Plots/%s/VertForce_ModelIV_%s.pdf"%(name,steps))
########

plt.figure(4)
emcee_corner = corner.corner(out3.flatchain, labels=out3.var_names,truths=list(out3.params.valuesdict().values()))
emcee_corner.savefig("Plots/%s/emcee_corner_%s.pdf"%(name,steps))
